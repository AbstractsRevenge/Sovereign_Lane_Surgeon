...
`allowed-deps -out <root> [-name <lane>] [-computed <file>] [-apply]` makes the lane **sovereign over its apex allow-list**. When a lane forks an updatable apex — `com.android.permission`, `com.android.mediaprovider` — that apex's `apex-allowed-deps-check` validates its whole dependency set against `packages/modules/common/build/allowed_deps.txt`. Every other stock surface a lane touches is routed by the finder, but this one is not: `build/soong/apex/apex_singleton.go` reads that path as a **raw source string** (`ExistentPathForSource`), not a module reference, and the finder only routes `Android.bp` *module* resolution. So the lane's forked apexes are gated by the stock file, and each new lane dependency would force an edit into stock — the coexistence-breaking move, and one that drifts against upstream forever.

The reader is what has to learn about the lane, since the finder cannot. Soong already appends and unions a second allow-list named by `EXTRA_ALLOWED_DEPS_TXT` (`build/make/core/soong_config.mk` → `ExtraAllowedDepsTxt`) before the diff, so the lane owns only its **delta** — the entries its forked apexes add beyond pristine upstream, which is exactly `computed_new-allowed-deps.txt − pristine_stock` (comment-stripped, sorted-unique). `allowed-deps` derives that from the newest `out*/.../soong/apex/depsinfo/new-allowed-deps.txt` (or `-computed`), writes it to `packages-<lane>/modules/common/build/allowed_deps.txt`, writes the sibling `allowed_deps.mk` that sets `EXTRA_ALLOWED_DEPS_TXT`, and inherits that `.mk` into each lane device product — leaving stock **pristine**. `create`'s device-product template already carries the `inherit-product-if-exists` line, so a freshly-created lane only needs one build then one `allowed-deps -apply`. Because the delta is derived, not hand-kept, the same command is the **drift-guard**: re-run it after any lane apex change and it reports what was added or removed. Preview by default; `-apply` commits (snapshotting each edited product `.mk` first). Verified on the Holo lane (android-15.0.0_r36): a 4-entry delta (`com.google.android.holo_holo`, `holo-wear-sdk-input-compat`, `holo-widget`, `holo-widgets`) moved out of stock into the lane, `apex-allowed-deps-check` green with stock restored to upstream.
