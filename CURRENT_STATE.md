...
0. [ ] Remove `assemble-super`'s prebuilt-packing branch (flashkit.go `planSuperAssembly`): it serves only trees seeded before the 2026-09-03 glue fix; drop it once every tree in the coverage table postdates the fix, keeping the flash-script half. Raised by an outside review, 2026-09-04.
