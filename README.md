...
- **Full images (2026-09-03), one build at a time, every one `preflight` FLASHABLE:** cheetah, panther, lynx, tangorpro, felix (gs201), oriole (gs101), husky (zuma) — komodo (zumapro) and tegu building. One device per SoC family is built deliberately: the seven remaining siblings share those trees and their gates are green, and the machine builds one image at a time.
...
- **One SoC generation proven.** Answered for three of them, as far as images can: oriole (gs101), the gs201 family, and husky (zuma) all built full images on the fixed bundle and passed every `preflight` check — including gs101's different layout (no init_boot, no vendor_kernel_boot, no system_dlkm), which preflight derives from that device's own factory image rather than assuming cheetah's. komodo (zumapro) and tegu (its own r31 tree) follow. Husky is also the evidence that this is worth doing: it needed three attempts, and both failures were toolkit bugs the analysis gate cannot see (a dropped symlink, a neverallow violation). A **boot** proof still exists only for gs201, and needs a phone.
