...
5. [ ] Cheetah camera — captured 2026-09-03 01:44Z with `aosp_runtime_log_capture` (Build Capture `out/aosp_runtime_log_capture/aosp17_cheetah/20260903T014421Z_boot_ok_camera-glitch/`: five logcat buffers, getprop, 32 tombstones, a sidecar with `dumpsys media.camera`, the lwis kernel lines and the HAL abort, two screenshots). Chain of evidence: `lwis-sensor-kraken: I2C Write failed … (-6)` (kernel, ENXIO) → Google camera HAL aborts `KRAKEN init error … LwisFence signal status: No such device or address` every ~5 s → camera service logs `ADD device 0/1` then `REMOVE` on each HAL restart, ending with "Number of camera devices: 0" → the AOSP Camera2 app dies with `ArrayIndexOutOfBoundsException: length=0` in `AndroidCameraDeviceInfo.getCharacteristics` ("Camera keeps stopping"). The other sensor (SANDWORM) initializes. Kernel, dtbo, first-stage modules and vendor_dlkm are the factory's byte for byte, so the next fact needed is whether this unit's rear camera works on stock.
