...
- **One SoC generation proven.** Answered as far as images can: oriole (gs101) and felix (gs201) both built full images on the fixed glue (2026-09-03) and passed every `preflight` check, including gs101's different layout — no init_boot, no vendor_kernel_boot, no system_dlkm — which preflight derives from that device's own factory image rather than assuming cheetah's. One build per remaining SoC family (zuma, zumapro, and tegu's r31 tree) follows. A boot proof still exists only for gs201, and needs a phone.
