...
// wrongCasedSelfRef reports whether name references the lane's own CamelCase prefix with the wrong
// casing (e.g. "holo2Foo" / "Nexusm..." where the convention is "Holo2Foo" / "NexusM..."): a
// case-insensitive prefix match on the camel token that is NOT an exact match — a rename-follow-
// through bug. Lane-parameterized so it works for any rename lane, not just nexusm.
func wrongCasedSelfRef(name, camel string) bool {
	n := strings.TrimPrefix(name, ":")
	return camel != "" && len(n) >= len(camel) &&
...
func classifyDep(name string, lane, oracle, stock map[string]bool, camel, lower string) (string, string) {
	if wrongCasedSelfRef(name, camel) {
		return "LANE-BUG", "lane's own module, wrong casing (should be " + camel + "...)"
	}
	...
	for _, f := range renameForms(name, camel, lower) {
...
func cmdDepLedger(args []string) int {
	...
	// The audited lane's rename convention: CamelCase installable prefix + lowercase lib prefix.
	camel, lower := *name, *name
	if *name != "" {
	...
	for _, r := range reports {
		disp, det := classifyDep(r.Name, lane, oracle, stock, camel, lower)
