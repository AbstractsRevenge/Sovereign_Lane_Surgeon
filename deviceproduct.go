...
# Lane-sovereign apex allowed-deps: sets EXTRA_ALLOWED_DEPS_TXT to the lane's own delta so
# the lane-forked updatable apexes are allow-listed WITHOUT editing stock
# packages/modules/common/build/allowed_deps.txt (a finder-blind raw-path read). The .mk +
# .txt are generated by 'sovereign-lane-surgeon allowed-deps -out <root> -name {{.Lane}}'
# after a build produces new-allowed-deps.txt; -if-exists keeps this a no-op until then.
$(call inherit-product-if-exists, packages-{{.Lane}}/modules/common/build/allowed_deps.mk)
